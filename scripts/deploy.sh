...
EB_BUCKET=elasticbeanstalk-us-east-1-633812586786
# Set the default region for aws cli
aws configure set default.region us-east-1
# Log in to ECR
eval $(aws ecr get-login --no-include-email --region us-east-1)
# Build docker image based on our production Dockerfile
docker build -t mikeandnick/mm .
# tag the image with the Travis-CI SHA
docker tag mikeandnick/mm:latest 633812586786.dkr.ecr.us-east-1.amazonaws.com/mm:$TRAVIS_COMMIT
# Push built image to ECS
docker push 633812586786.dkr.ecr.us-east-1.amazonaws.com/mm:$TRAVIS_COMMIT
